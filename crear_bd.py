...
carpeta_results = 'resultados_ML_connected'
...
df_filtrado_pasj = df[df['PASAJEROS'] != 0]
df_filtrado_nac = df_filtrado_pasj[df['NAC'] != "INTERNACIONAL"]
df_filtrado = df_filtrado_nac[df['CARGA (Ton)'] != 0]
...
archivo_salida = os.path.join(carpeta_results, 'BD_Trafico_aereo_filtrado.xlsx')
df_filtrado.to_excel(archivo_salida, index=False)
